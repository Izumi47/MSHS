...
<body>
						...
						<div class="row">
							<div class="col-xl-12">
								<div class="page-title flex-wrap">
									...
									<div>
										...
										<a href="admins-add-teacher.php"><button type="button" class="btn btn-primary">
										 + New Teacher
										</button></a>
									</div>
								</div>
							...
							<div class="col-xl-12 wow fadeInUp" data-wow-delay="1s">
								...
								<table class="table-responsive-lg table display dataTablesCard student-tab dataTable no-footer" id="example-student">
									...
									<tbody>
										<?php include 'php/admins-dashboard.php'; ?>
									</tbody>
								</table>
								</div>
							</div>
		...
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<p>Are you sure to delete this user's data from the database?</p>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-primary" data-bs-dismiss="modal">No</button>
					<a href="php/delete-user.php?id=<?php echo $row['id']; ?>" id="confirmDeleteButton" class="btn btn-primary">Yes</a>
				</div>
			</div>
		</div>
	</div>
    ...
    ***********************************-->
	...
	<script>
		...
		const table = document.getElementById("example-student");
	...
	<script>
		document.addEventListener('DOMContentLoaded', function () {
			...
			table.addEventListener('click', function (event) {
				// Traverse up from the target until you find a button with class 'delete-btn'
				let targetElement = event.target;
				while (targetElement != null && !targetElement.classList.contains('delete-btn')) {
					targetElement = targetElement.parentElement;
				}

				// If a delete button was found
				if (targetElement && targetElement.classList.contains('delete-btn')) {
					event.preventDefault();

					// Get user ID from data attribute
					const userId = targetElement.getAttribute('id');

					// Set the href for the confirmation button
					const confirmDeleteButton = document.getElementById('confirmDeleteButton');
					confirmDeleteButton.href = `php/admins-delete-user.php?id=${userId}`;
