...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $insertQuery = "INSERT INTO students (name, contact, id, email, classroom, address, cgpa, bm, english, sejarah, maths, science, physical, interests, ambition, class_stream)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($insertQuery);
    
    if ($stmt) {
        $stmt->bind_param("ssssssssssssssss", $name, $contact, $id, $email, $classroom, $address, $cgpa, $bm, $english, $sejarah, $maths, $science, $physical, $interests, $ambition, $classStream);
