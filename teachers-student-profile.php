								...
								<div class="d-flex justify-content-between">
									<div class="user">
										...
										<div>
											...
											<p class="text-primary font-w600"><?php echo htmlspecialchars($studentData['class_stream'] ?? 'Class Stream not available'); ?> Student</p>
										</div>
									</div>
									<div class="dropdown custom-dropdown">
										...
										<div class="dropdown-menu dropdown-menu-end">
											<a class="dropdown-item" href="teachers-edit-student-profile.php?id=<?php echo $studentData['id']; ?>">Edit Details</a>
    ...
    ***********************************-->
	...
	<script>
		...
		document.getElementById('saveChangesButton').addEventListener('click', function() {
			var studentId = "<?php echo $studentId; ?>";
			var updateUrl = "php/teachers-predict.php";
			
			// Extract the class stream from the modal content
			var modalContent = document.getElementById('predictionModalBody').textContent;
			var streamStart = "Suitable Class Stream: ";
			var streamEnd = "Model Accuracy:"; // Assuming "Model Accuracy" always follows the class stream
			var startIdx = modalContent.indexOf(streamStart) + streamStart.length;
			var endIdx = modalContent.indexOf(streamEnd);
			var classStream = modalContent.substring(startIdx, endIdx).trim(); // Extract and trim the class stream

			var xhttpUpdate = new XMLHttpRequest();
			xhttpUpdate.onreadystatechange = function() {
				if (this.readyState == 4 && this.status == 200) {
					// Handle successful update response
					window.location.href = "teachers-student-profile.php<?php echo "?id=" . $studentId; ?>";
					// Additional code to handle the response
				}
			};

			xhttpUpdate.open("POST", updateUrl, true);
			xhttpUpdate.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
			xhttpUpdate.send("studentId=" + encodeURIComponent(studentId) + "&classStream=" + encodeURIComponent(classStream));
		});
	</script>
